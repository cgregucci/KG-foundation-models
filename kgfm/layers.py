...
from kgfm.util import static_positional_encoding, coo_to_csr, preprocess_triton_hypergraph
...
class HypergraphLayer(nn.Module):
    ...
    def _triton_kernel_apply(self, node_features_flatten, rowptr, indices, node_size,
                              etypes, relation_vector, pos_embedding, pos_index):
        from .rspmm.triton_rspmm import HyperRelConvSumAggr, HyperRelConvMeanAggr
        if self.aggregate_func == "sum":
            return HyperRelConvSumAggr.apply(node_features_flatten, rowptr, indices, node_size,
                                             etypes, relation_vector, pos_embedding, pos_index, 0)
        elif self.aggregate_func == "mean":
            return HyperRelConvMeanAggr.apply(node_features_flatten, rowptr, indices, node_size,
                                              etypes, relation_vector, pos_embedding, pos_index, 0)
        raise ValueError("For now, the Triton kernel only supports sum and mean aggr. "
    ...
    def forward(self, node_features, query, edge_list, rel):
        ...
        if self.use_triton:
            ...
            out = self._triton_kernel_apply(node_features_flatten, rowptr, indices, node_size,
                                            etypes, relation_vector, pos_embedding, pos_index)
    ...
    def forward_sharded(self, node_features, query, hypergraph_shards):
        """Sum-aggregation chunked across (arity-2 + 4 arity-3) motif shards.

        Mirrors ``forward`` for ``use_triton=True``, except the Triton kernel runs
        once per shard and per-destination outputs are summed before the linear /
        dropout / norm tail. Sum-aggregation is associative across edge subsets,
        so this is equivalent to the unsharded path up to fp reorder. Used for
        AristoV4-MOTIF where the full hypergraph does not fit on the GPU.
        """
        assert self.use_triton, "forward_sharded only implements the Triton path"
        assert self.aggregate_func == "sum", \
            "forward_sharded only supports sum aggregation (mean would need a count tracker)"
        assert not self.dependent, "forward_sharded does not support dependent relation linear"

        self.pos_embedding.weight.data[0] = torch.ones(self.in_channels)
        batch_size, node_size, input_dim = node_features.shape
        device = node_features.device

        node_features[:, 0, :] = 0  # padding node
        node_features_flatten = node_features.transpose(0, 1).flatten(1)

        pos_embedding = self.pos_embedding.weight.unsqueeze(1).expand(-1, batch_size, -1).flatten(1)
        relation_vector = self.rel_embedding.weight.unsqueeze(1).expand(-1, batch_size, -1).flatten(1).transpose(0, 1)

        # arity-2 shard: HypergraphShards.to(device) moved a2 to `device` already.
        # preprocess expects [max_arity, E]; +1 shifts -1 padding to the kernel's 0.
        a2 = hypergraph_shards.arity2
        a2_edge_list = a2.edge_index + 1
        rowptr, indices, etypes, pos_index, _ = preprocess_triton_hypergraph(
            a2_edge_list, a2.edge_type, num_node=node_size
        )
        out = self._triton_kernel_apply(node_features_flatten, rowptr, indices, node_size,
                                        etypes, relation_vector, pos_embedding, pos_index)
        # Free arity-2 GPU intermediates before arity-3 chunks need the headroom.
        # On AristoV4 the arity-2 portion's preprocessed CSR is ~10-30 GB; without
        # this the 24 GB arity-3 chunks + ~30 GB arity-2 residual exceed the 40 GB
        # A100. preprocess_triton_hypergraph is recomputed on each forward, so
        # there's no value in keeping these alive past the kernel call.
        del a2_edge_list, rowptr, indices, etypes, pos_index, _

        # arity-3 shards: precomputed CSR streamed from CPU memory.
        # `del csr` at the end of each iteration releases the previous chunk's
        # GPU memory back to PyTorch's caching allocator BEFORE the generator's
        # next() runs `to(device)` for the next chunk. Without del, the loop
        # variable holds the old 24 GB chunk while the new 24 GB is being
        # allocated, doubling peak GPU memory.
        for csr in hypergraph_shards.iter_arity3_csr(device):
            out = out + self._triton_kernel_apply(
                node_features_flatten, csr["rowptr"], csr["indices"], node_size,
                csr["etypes"], relation_vector, pos_embedding, csr["pos_index"]
            )
            del csr
