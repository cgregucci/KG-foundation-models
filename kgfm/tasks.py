...
def build_relation_graph(graph):
    ...
    # Use the actual tensor device, not graph.device — the latter is the
    # stale attribute set at preprocess time and is wrong after .to(device)
    # at training time (ultra.py:181 rebuilds the relation_graph per batch
    # and would otherwise demote GPU tensors back to CPU).
    device = edge_index.device
    num_rels = torch.tensor(num_rels, device=device) if not torch.is_tensor(num_rels) else num_rels.to(device)
...
def build_relation_hypergraph(graph, max_arity=3):
    ...
    edge_index, edge_type = graph.edge_index, graph.edge_type
    num_nodes, num_rels = graph.num_nodes, graph.num_relations
    # See note on device in build_relation_graph: prefer edge_index.device
    # over graph.device so the per-batch rebuild during training stays on
    # the actual tensor device.
    device = edge_index.device
...
def build_relation_hypergraph_synth(graph, max_arity):
    ...
    edge_index, edge_type = graph.edge_index, graph.edge_type
    num_nodes, num_rels = graph.num_nodes, graph.num_relations
    # See note on device in build_relation_graph.
    device = edge_index.device
    num_rels = torch.tensor(num_rels, device=device) if type(num_rels) is int else num_rels.clone().detach().to(device)
    edge_index = edge_index.to(device)
    edge_type = edge_type.to(device)
...
# ---------------------------------------------------------------------------
# TRIX-specific primitives (lifted from yuchengz99/TRIX/src/trix/tasks.py).
# Ranking / negative sampling for the relation-prediction task, plus the
# four-graph relation-adjacency builder consumed by trix_entity / trix_relation
# models. TRIX's `build_relation_graph` is renamed to `build_relation_adj` here
# to avoid shadowing MOTIF's `build_relation_graph` above (which produces a
# different field on the data object: `relation_graph` vs `relation_adj`).
# ---------------------------------------------------------------------------
...
def build_relation_adj(graph):
    """Populate `graph.relation_adj` with four Data objects keyed hh/ht/th/tt.

    Lifted verbatim from TRIX's `build_relation_graph`; renamed to keep MOTIF's
    `build_relation_graph` (which produces `graph.relation_graph` instead) intact.

    Runs in two contexts:
    - Pre-collate, as part of `kgfm/datasets.py`'s `pre_transform` chain — bakes
      `relation_adj` into `kg-datasets/<name>/processed/data.pt` alongside
      `relation_graph` and `relation_hypergraph`. This is the normal path on
      freshly-processed caches.
    - Post-collate, via `script/run.py:ensure_relation_adj` (and equivalents in
      run_relation/run_many*/pretrain*) — a runtime fallback for stale caches
      that pre-date relation_adj being baked in. No-op on fresh caches.

    The int/tensor cast below supports both call sites.
    """
    edge_index, edge_type = graph.edge_index, graph.edge_type
    # After PyG's dataset.collate(...), `num_relations` (and sometimes `num_nodes`)
    # is promoted from int to a 1-element tensor. TRIX upstream's `build_relation_graph`
    # runs *before* collate so it sees plain ints; here we run at runtime *after*
    # the dataset is loaded, so we must cast back. If we don't, the resulting
    # `relation_adj['hh'].num_nodes` becomes a tensor and PyG MessagePassing's
    # shape inference inside RelNet silently misbehaves — visible as a multi-point
    # MRR drop on FB15k237 (e.g. 0.318 vs the expected 0.362).
    num_node = int(graph.num_nodes) if torch.is_tensor(graph.num_nodes) else graph.num_nodes
    num_relation = int(graph.num_relations) if torch.is_tensor(graph.num_relations) else graph.num_relations
    device = edge_index.device
    ...
    relation_adj["hh"] = Data(
        edge_index=torch.tensor(index_hh[0:2], dtype=torch.int64, device=device),
        edge_type=torch.tensor(index_hh[2], dtype=torch.int64, device=device),
        num_nodes=num_relation,
        num_relations=num_node,
    )
    relation_adj["ht"] = Data(
        edge_index=torch.tensor(index_ht[0:2], dtype=torch.int64, device=device),
        edge_type=torch.tensor(index_ht[2], dtype=torch.int64, device=device),
        num_nodes=num_relation,
        num_relations=num_node,
    )
    relation_adj["th"] = Data(
        edge_index=torch.tensor(index_th[0:2], dtype=torch.int64, device=device),
        edge_type=torch.tensor(index_th[2], dtype=torch.int64, device=device),
        num_nodes=num_relation,
        num_relations=num_node,
    )
    relation_adj["tt"] = Data(
        edge_index=torch.tensor(index_tt[0:2], dtype=torch.int64, device=device),
        edge_type=torch.tensor(index_tt[2], dtype=torch.int64, device=device),
        num_nodes=num_relation,
        num_relations=num_node,
    )
