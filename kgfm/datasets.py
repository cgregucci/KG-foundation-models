...
from kgfm.tasks import build_relation_graph, build_relation_hypergraph, build_relation_adj


# Each dataset class bakes exactly one relation structure into its on-disk
# cache. The cache_key picks which builder runs in process() and is the
# suffix of the cached filename (data_<cache_key>.pt). For "relation_graph_entity"
# the file holds data.relation_adj — the filename and attribute name diverge;
# both TRIXEntity and TRIXRelation read the same file.
BUILDERS = {
    "relation_graph":         build_relation_graph,
    "relation_hypergraph":    build_relation_hypergraph,
    "relation_graph_entity":  build_relation_adj,
}
DEFAULT_CACHE_KEY = "relation_graph"


def _resolve_builder(cache_key):
    if cache_key not in BUILDERS:
        raise KeyError(
            f"unknown cache_key {cache_key!r}; expected one of {sorted(BUILDERS)}"
        )
...
class GrailInductiveDataset(InMemoryDataset):
    ...
    def __init__(self, device, root, version, transform=None,
                 pre_transform=None,
                 merge_valid_test=True, cache_key=DEFAULT_CACHE_KEY):
        ...
        if pre_transform is None:
            pre_transform = _resolve_builder(cache_key)
    ...
    def processed_file_names(self):
        return f"data_{self.cache_key}.pt"
...
class FB15k237Inductive(GrailInductiveDataset):
    ...
    def __init__(self, root, version, device, cache_key=DEFAULT_CACHE_KEY):
        super().__init__(root=root, version=version, device=device, cache_key=cache_key)
...
class WN18RRInductive(GrailInductiveDataset):
    ...
    def __init__(self, root, version, device, cache_key=DEFAULT_CACHE_KEY):
        super().__init__(root=root, device=device, version=version, cache_key=cache_key)
...
def FB15k237(root, device, cache_key=DEFAULT_CACHE_KEY):
    ...
    builder = _resolve_builder(cache_key)
...
def WN18RR(root, device, cache_key=DEFAULT_CACHE_KEY):
...
class TransductiveDataset(InMemoryDataset):
    ...
    def __init__(self, root, device, transform=None, pre_transform=None,
                 cache_key=DEFAULT_CACHE_KEY, **kwargs):
        self.device = device
        self.cache_key = cache_key
        if pre_transform is None:
            pre_transform = _resolve_builder(cache_key)
...
class CoDExSmall(CoDEx):
    ...
    def __init__(self, device, root, cache_key=DEFAULT_CACHE_KEY):
        super(CoDExSmall, self).__init__(root=root, device=device, cache_key=cache_key, size='s')
...
class CoDExMedium(CoDEx):
    ...
    name = "codex-m"
    def __init__(self, device, root, cache_key=DEFAULT_CACHE_KEY):
        super(CoDExMedium, self).__init__(root=root, device=device, cache_key=cache_key, size='m')
...
class CoDExLarge(CoDEx):
    ...
    name = "codex-l"
    def __init__(self, device, root, cache_key=DEFAULT_CACHE_KEY):
        super(CoDExLarge, self).__init__(root=root, device=device, cache_key=cache_key, size='l')


class NELL995(TransductiveDataset):

    # from the RED-GNN paper https://github.com/LARS-research/RED-GNN/tree/main/Static/transductive/data/nell
    ...
    urls = [
        "https://raw.githubusercontent.com/LARS-research/RED-GNN/main/Static/transductive/data/nell/facts.txt",
        "https://raw.githubusercontent.com/LARS-research/RED-GNN/main/Static/transductive/data/nell/train.txt",
        "https://raw.githubusercontent.com/LARS-research/RED-GNN/main/Static/transductive/data/nell/valid.txt",
        "https://raw.githubusercontent.com/LARS-research/RED-GNN/main/Static/transductive/data/nell/test.txt",
    ]
    name = "nell995"
...
class InductiveDataset(InMemoryDataset):
    ...
    def __init__(self, root, version, device, transform=None, pre_transform=None,
                 cache_key=DEFAULT_CACHE_KEY, **kwargs):
...
class ILPC2022SmallInductive(ILPC2022):
    ...
    def __init__(self, root, device, **kwargs):
        kwargs.setdefault("cache_key", DEFAULT_CACHE_KEY)
        # The inductive YAML always renders `version: {{ version }}` for the
        # whole bucket; for ILPC the spec has no `:version` suffix so the
        # rendered value is None. Drop it — the version is hardcoded below.
        kwargs.pop("version", None)
        super(ILPC2022SmallInductive, self).__init__(root=root, device=device, version="small", **kwargs)

class ILPC2022LargeInductive(ILPC2022):

    def __init__(self, root, device, **kwargs):
        kwargs.setdefault("cache_key", DEFAULT_CACHE_KEY)
        kwargs.pop("version", None)  # see ILPC2022SmallInductive
        super(ILPC2022LargeInductive, self).__init__(root=root, device=device, version="large", **kwargs)
...
class JointDataset(InMemoryDataset):
    ...
    def __init__(self, root, graphs, device, transform=None, pre_transform=None,
        ...
        self.graphs = [
            self.datasets_map[ds.split(":")[0]](
                root=root, version=ds.split(":")[1], device=device, cache_key=cache_key
            ) if ":" in ds else
            self.datasets_map[ds](root=root, device=device, cache_key=cache_key)
            for ds in graphs
        ]
        self.num_graphs = len(graphs)
        super().__init__(root, transform, pre_transform)
        self.data = torch.load(self.processed_paths[0], map_location=self.device)
