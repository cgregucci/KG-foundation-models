...
from kgfm import tasks, layers
from kgfm.base_nbfnet import BaseNBFNet

class TRIXEntity(nn.Module):
    """
    The published `entity_prediction.pth` checkpoint stores the two entity
    networks as `entity_model_mini` (2 layers, used inside the relation model
    for the auxiliary refinement pass) and `entity_model` (4 layers, used to
    score). Upstream `src/trix/models_entity.py` calls them
    `entity_model_1` / `entity_model_2` instead — checkpoint and source
    diverged. We keep the constructor signature compatible with the YAML
    (`entity_model_1_cfg`, `entity_model_2_cfg` in `config/trix/run_entity_*.yaml`)
    but store them under the names the checkpoint expects.
    """

    def __init__(self, rel_model_cfg, entity_model_1_cfg, entity_model_2_cfg):
        super(TRIXEntity, self).__init__()

        self.relation_model = RelNet(**rel_model_cfg)
        self.entity_model_mini = EntityNet(**entity_model_1_cfg)  # 2-layer
        self.entity_model = EntityNet(**entity_model_2_cfg)       # 4-layer scorer
    ...
    def forward(self, data, batch, precomputed_rel_emb=None):
        ...
        # (see kgfm/models/motif.py:25). TRIXEntity's relation features come from
        # its iterative RelNet, not the ULTRA-style relation graph the cache was
        # built for, so feeding it here is a no-op — assert it isn't provided.
        assert precomputed_rel_emb is None, \
            "TRIXEntity does not support precomputed_rel_emb (relation features come from RelNet)"
        relation_representations = self.relation_model(data, batch, self.entity_model_mini)
        score = self.entity_model(data, relation_representations, batch)["score"]
        return score
        ...
        for i in range(len(self.dims) - 1):
            self.layers_hh.append(
                layers.GeneralizedRelationalConv(
                    ...
                    self.activation, dependent=False, project_relations=True, use_triton=False)
                )
