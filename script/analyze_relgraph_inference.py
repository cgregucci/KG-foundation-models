#!/usr/bin/env python3
"""Per-triple relation-graph induction analysis (no model, CPU multiprocessing).

For every test triple in each dataset passed via -d, compute

    nadded = |R1 \\ R0|

for both the tail (forward) and head (inverse) directions, where R0 is the
ULTRA relation graph induced from test_data and R1 adds the test triple to
test_data. Group by visibility quadrant (SQSA / SQUA / UQSA / UQUA) and write
two artifacts per run:

  - runs/relgraph_visibility_<name>.csv: combined per-(dataset, split) summary.
    Multiple -d invocations accumulate into the same CSV.
  - <out-dir>/<dataset>/per_triple_nadded.tsv: raw rows, two per test triple.

Datasets are passed in Name[:version] form (e.g. FB15k237, FB15k237Inductive:v1).
"""
...
from kgfm.relgraph_inference import (
    Row,
...
def parse_entries(arg: str) -> List[Tuple[str, str]]:
    """Parse "Name1,Name2:vX,..." -> [(name, version_or_None), ...]."""
    if not arg:
        return []
...
def append_summary_csv(csv_path: Path, benchmark: str, summary: dict) -> None:
    """Append (benchmark, split) rows to a CSV. Write header if file is new."""
...
def print_summary(benchmark: str, summary: dict) -> None:
    ...
    print(f"  {'split':>6}  {'n_total':>8}  {'n_full':>8}  {'n_w_add':>8}  "
          f"{'n_add_t':>8}  {'%w_add':>8}  {'nadd_mean':>10}  "
          f"{'nadd_med':>9}  {'nadd_std':>9}")
    for split in ["Orig", *QUADRANT_NAMES]:
        e = summary[split]
        print(f"  {split:>6}  {e['n_total']:>8}  {e['n_full']:>8}  "
              f"{e['n_with_added']:>8}  {e['n_added_total']:>8}  "
...
def main():
    ...
    parser.add_argument("--split", default="test", choices=["train", "valid", "test"],
                        help="which split's target triples to analyze (default: %(default)s)")
    parser.add_argument("--num-workers", type=int, default=1,
                        help="multiprocessing pool size (default: 1)")
    parser.add_argument("--out-dir", default=str(DEFAULT_RUNS_DIR / "relgraph_inference"),
                        help="directory for per-triple TSVs (default: %(default)s)")
    parser.add_argument("--csv", default=str(DEFAULT_RUNS_DIR / "relgraph_visibility.csv"),
    ...
    print(f"Split: {args.split}    Workers: {args.num_workers}")
