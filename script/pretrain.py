...
def train_and_validate(cfg, model, train_data, valid_data, filtered_data=None, batch_per_epoch=None):
    ...
    if world_size > 1:
        # TRIX-family models call entity_model_1 inside relation_model's mid-pass
        # but only consume its `feature` output, leaving entity_model_1.mlp params
        # without gradients. Newer PyTorch DDP raises RuntimeError unless
        # find_unused_parameters=True. Upstream TRIX runs on older PyTorch where
        # this was a warning, not an error.
        find_unused = cfg.model["class"] in {"TRIXEntity", "TRIXNoIter"}
        parallel_model = nn.parallel.DistributedDataParallel(
            model, device_ids=[device], find_unused_parameters=find_unused
        )
    else:
    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            # tqdm only on rank 0 to match upstream's clean log style
            # (each rank otherwise prints its own progress bar, which 4xs the noise).
            for batch in tqdm(islice(train_loader, batch_per_epoch),
                              disable=util.get_rank() != 0):
...
if __name__ == "__main__":
    ...
    if cfg.model["class"] in {"TRIXEntity", "TRIXNoIter"}:
        for splits in (train_data, valid_data, test_data):
            for d in splits:
                if not hasattr(d, "relation_adj") or d.relation_adj is None:
                    tasks.build_relation_adj(d)

    # Optional: deterministic random initialization of model weights
    # (separate from training seed). Used by random-frozen-backbone runs.
    if "random_init_seed" in cfg.model and cfg.model.random_init_seed is not None:
        init_seed = int(cfg.model.random_init_seed) + util.get_rank()
    ...
    elif cfg.model["class"] == "MOTIF":
        model = MOTIF(
            rel_model_cfg=cfg.model.relation_model,
            entity_model_cfg=cfg.model.entity_model,
        )
    elif cfg.model["class"] == "TRIXEntity":
        model = TRIXEntity(
            rel_model_cfg=cfg.model.relation_model,
            entity_model_1_cfg=cfg.model.entity_model_1,
            entity_model_2_cfg=cfg.model.entity_model_2,
        )
    elif cfg.model["class"] == "TRIXNoIter":
        model = TRIXNoIter(
            rel_model_cfg=cfg.model.relation_model,
